...
class Visualization:
    ...
    def plot_asset_table(self):
        plt.figure(figsize=(10, len(self.cb_assets)*0.5 + 1))
    ...
    def portfolio_breakdown_pie_chart(self):
        ...
        plt.title("Assets", fontsize=16, fontfamily='monospace')
        plt.tight_layout()
        plt.show(block=False)
    ...
    # table_data.append(
        #     [
        #         "Total Balance", 
        #         f"{get_currency_symbol(self.currency)}{float(self.total_balance):,.2f}",
        #         f"{get_currency_symbol(self.currency)}{float(self.total_cost_basis):,.2f}",
        #         "N\A"
        #     ]
        # )
